package ru.kamchatgtu.studium.restclient.urlservice;

public class URLSubjectService {
    private static final String URL = "/restful/subject_service";
    public static final String URL_SUBJECT = URL + "/subject";
    public static final String URL_SUBJECTS = URL + "/subjects";
    public static final String URL_SUBJECTS_BY_GROUP = URL + "/subjects_by_group";
    public static final String URL_ADD = URL + "/add";
    public static final String URL_DELETE = URL + "/delete";
    public static final String URL_UPDATE = URL + "/update";
}
